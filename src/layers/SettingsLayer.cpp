...
bool SettingsLayer::init() {
    ...
    auto appearanceCheckBoxSprite = CCSprite::createWithSpriteFrameName(Mod::get()->getSavedValue<bool>("new-appearance") ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png");
    auto appearanceCheckBoxSprite2 = CCSprite::createWithSpriteFrameName(!Mod::get()->getSavedValue<bool>("new-appearance") ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png");
    ...
    auto disAnimCheckBoxSprite = CCSprite::createWithSpriteFrameName(Mod::get()->getSavedValue<bool>("dis-anim") ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png");
    auto disAnimCheckBoxSprite2 = CCSprite::createWithSpriteFrameName(!Mod::get()->getSavedValue<bool>("dis-anim") ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png");
    ...
    std::stringstream ss;
    ss << Mod::get()->getSavedValue<float>("animation-length");
    ...
    ss.str("");
    ss << Mod::get()->getSavedValue<float>("animation-scale");
    ...
    auto splashAmount = Mod::get()->getSavedValue<int>("amount-of-splashes");
    std::stringstream ss_text;

    for(int i = 0; i < splashAmount; i++) {
        ss.str("");
        ss << "splash-" << i;
        auto splash = Mod::get()->getSavedValue<std::string>(ss.str().c_str());
        ss << "-size";
        auto splash_size = Mod::get()->getSavedValue<std::string>(ss.str().c_str());
        ss.str("");
...
void SettingsLayer::onBack(CCObject* sender) {
    ...
    if(!Mod::get()->getSavedValue<bool>("dis-anim")) {
        if(!animation_speed.empty()) {
            ...
            splash->setScale(std::stof(Mod::get()->getSavedValue<std::string>(ss.str().c_str())));
            splash->runAction(CCRepeatForever::create(CCSequence::create(
                CCEaseOut::create(CCScaleTo::create(std::stof(animation_speed), splash->getScale() + Mod::get()->getSavedValue<float>("animation-scale")), std::stof(animation_speed)),
                CCEaseOut::create(CCScaleTo::create(std::stof(animation_speed), splash->getScale()), std::stof(animation_speed)),
                nullptr
            )))->setTag(1);
            Mod::get()->setSavedValue<float>("animation-length", std::stof(animation_speed));
        }
        ...
        if(!animation_scale.empty()) {
            ...
            splash->setScale(std::stof(Mod::get()->getSavedValue<std::string>(ss.str().c_str())));
            splash->runAction(CCRepeatForever::create(CCSequence::create(
                CCEaseOut::create(CCScaleTo::create(Mod::get()->getSavedValue<float>("animation-length"), splash->getScale() + std::stof(animation_scale)), Mod::get()->getSavedValue<float>("animation-length")),
                CCEaseOut::create(CCScaleTo::create(Mod::get()->getSavedValue<float>("animation-length"), splash->getScale()), Mod::get()->getSavedValue<float>("animation-length")),
                nullptr
            )))->setTag(1);
            Mod::get()->setSavedValue<float>("animation-scale", std::stof(animation_scale));
        }
    } else {
        if(!animation_speed.empty()) {
            Mod::get()->setSavedValue<float>("animation-length", std::stof(animation_speed));
...
void SettingsLayer::onAppearanceChecked(CCObject* sender) {
    ...
    if(!Mod::get()->getSavedValue<bool>("new-appearance", true)) {
        Mod::get()->setSavedValue<bool>("new-appearance", true);
...
void SettingsLayer::onDisAnimChecked(CCObject* sender) {
    ...
    if(!Mod::get()->getSavedValue<bool>("dis-anim")) {
        splash->stopActionByTag(1);
        splash->runAction(CCSequence::create(
            CCEaseOut::create(CCScaleTo::create(Mod::get()->getSavedValue<float>("animation-length"), std::stof(Mod::get()->getSavedValue<std::string>(ss.str().c_str()))), Mod::get()->getSavedValue<float>("animation-length")),
            nullptr
        ));
        Mod::get()->setSavedValue<bool>("dis-anim", true);
    } else {
        splash->runAction(CCRepeatForever::create(CCSequence::create(
            CCEaseOut::create(CCScaleTo::create(Mod::get()->getSavedValue<float>("animation-length"), std::stof(Mod::get()->getSavedValue<std::string>(ss.str().c_str())) + Mod::get()->getSavedValue<float>("animation-scale")), Mod::get()->getSavedValue<float>("animation-speed")),
            CCEaseOut::create(CCScaleTo::create(Mod::get()->getSavedValue<float>("animation-length"), std::stof(Mod::get()->getSavedValue<std::string>(ss.str().c_str()))), Mod::get()->getSavedValue<float>("animation-length")),
            nullptr
        )))->setTag(1);
